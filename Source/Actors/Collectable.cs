...
public abstract class Collectable : Actor, IHaveModels, IHaveSprites, IPickup, ICastPointShadow
{
	...
	public Collectable(string id, bool isLocked, string? unlockCondition, bool unlockSound, Vec3? bubbleTo, SkinnedModel model)
	{
		ID = id;
		IsLocked = isLocked;
	...
	public override void Update()
	{
		if (IsLocked)
		{
			if (!Time.OnInterval(0.1f, checkConditionsOffset)) return;
			
			var ready = !unlockConditions.Any(it => it.Alive && !(it as IUnlockStrawberry)!.Satisfied);
	...
	{
		if (IsCollected || isCollecting || IsLocked) return;
